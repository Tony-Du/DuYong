...
object JSON {
  ...
  implicit def ScalaJSONToString(s: ScalaJSON): String = s.toString

  implicit def ScalaJSONToInt(s: ScalaJSON): Int = s.toInt

  implicit def ScalaJSONToDouble(s: ScalaJSON): Double = s.toDouble
}
