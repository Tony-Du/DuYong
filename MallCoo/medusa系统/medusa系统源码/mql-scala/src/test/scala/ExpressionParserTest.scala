/**
  * Created by wm on 2017-09 11:54.
  */
object ExpressionParserTest {
  ...
  object Tree {
    def decompose(tree: Tree, parts: DecomposeTree): DecomposeTree = tree match {
      case a: AggregateNode => parts.copy(j = parts.j + 1, root = s"${parts.root}x${parts.i}${parts.j}", parts = a.children.head.toString + s" x${parts.i}${parts.j}" :: parts.parts)
      case b: BinaryNode =>
  ...
  case class AggregateNode(value: Option[String], children: List[Tree], level: Int) extends Tree {
    override def toString: String = value.getOrElse("None") + (if (children.isEmpty) "" else children.mkString("(", ", ", ")"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val a =
    //      """
    //        |avg(greatest(_rent_distinct_sum__month,(_sales_point_distinct_sum__month)/100*total_amount_sum_1001_month))/avg(total_amount_sum_1001_month)
    //      """.stripMargin
    val a =
    """
      sum(greatest(_sales_point_distinct_sum__month/100*total_amount_sum_1001_month-_rent_distinct_sum__month,0))
    """.stripMargin


    println(a)

    val tokens = reg.findAllIn(a).toList

    def render(tokens: List[String], tree: Tree, level: Int): Tree = tokens match {
      case Nil => tree
      case token :: tail =>
        ...
        render(tail, n._1, n._2)
    }

    val r = render(tokens, FunctionNode(None, Nil, 0), 0)

    println("tree: ", r)
    println("without arg: ", r.toStringWithoutAgg)

    val d = Tree.decompose(r, DecomposeTree(1, 1, "", Nil))
    println("decompose: ", d.root)
    d.parts.foreach(println)
  }
}
