...
public class BaseTable4VideoCollectSource {

    private static BaseTable4VideoCollectSource cs = new BaseTable4VideoCollectSource();

    private BaseTable4VideoCollectSource() {

    }

    private final static String BASE_RELATIVE_DIRECTORY = ReportLocationConstants.MIGU_ORIGIN;

    public static BaseTable4VideoCollectSource get() {
        return cs;
    }
    ...
    public List<Path> getInputs(Path base, Date startDate, Configuration conf) throws IOException {
        ...
        if (base == null)
            base = new Path(BaseTable4VideoCollectSource.BASE_RELATIVE_DIRECTORY);
        ...
        for (FileStatus fv : fvs) {
            if (fv.toString().contains("td_aaa_tourist_bill_d")) {
                ...
                Path timep = new Path(fv.getPath(), GnomeDateFormat.FORMAT_TILL_DATE_C.format(startDate));
                lst.add(timep);
            } else if (fv.toString().contains("td_cms_content_d")) {
                Path timep = fv.getPath();
                lst.add(timep);
            } else if (fv.toString().contains("td_cms_content_class_d")) {
                Path timep = fv.getPath();
                lst.add(timep);
            }
