...
import com.iflytek.gnome.analysis.mapreduce.MRGenerDenoUnk;
...
public class GenerDenoUnk4CRMain extends OozieMain implements Tool{
	
	public static final Log LOG = LogFactory.getLog(GenerDenoUnk4CRMain.class);
	public static final String base = "online/vc_log/extract";
	public static final String reportName = "GenerDenoUnk4CRDaily";
...
//		conf.setInt("mapred.reduce.tasks", 1);
        ...
        job.setJobName(GenerDenoUnk4CRMain.class.getSimpleName() + ":" + GnomeDateFormat.FORMAT_TILL_DATE.format(startDate));
//        job.setInputFormatClass();
        job.setMapperClass(MRGenerDenoUnk.M1.class);
        job.setReducerClass(MRGenerDenoUnk.R1.class);
...
//                	job.addCacheFile(fss[0].getPath().toUri());
		...
		if (StringUtils.isBlank(userName)) {
			outputPath = new Path(new Path(ComplainCollectSource.base),
				"td_denominator_unknown" + "/" + GnomeDateFormat.FORMAT_TILL_DATE_C.format(startDate));  
		} else {
        ...
        /*** 设置mapReduce输出数据路径 ***/
		Path tmpOutput = new Path(ConstantsUtils.getTmpSegmentDir(new Path(GenerDenoUnk4CRMain.base)), reportName + tmpChildDir);
        ...
        job2.setJobName(GenerDenoUnk4CRMain.class.getSimpleName() + "_2" + ":" + GnomeDateFormat.FORMAT_TILL_DATE.format(startDate));
//        job2.setInputFormatClass();
        job2.setMapperClass(MRGenerDenoUnk.M2.class);
        job2.setReducerClass(MRGenerDenoUnk.R2.class);
...
//		job2.setOutputFormatClass();
		...
		Path tmpOutput2 = new Path(ConstantsUtils.getTmpSegmentDir(new Path(GenerDenoUnk4CRMain.base)), reportName + "2_" + tmpChildDir);
    ...
    {
        int res = OozieToolRunner.run(null, new GenerDenoUnk4CRMain(), args);
        System.exit(res);
    }
