...
import com.iflytek.gnome.analysis.mapreduce.MRPlayView;
import com.iflytek.gnome.analysis.source.PlayViewCollectSource;
 ...
 *
 */
public class ViewPlayReportMain extends ReportOozieMain implements Tool {

    private static final Log LOG = LogFactory.getLog(ViewPlayReportMain.class);
    public static final String base = ReportLocationConstants.REPORT_TEMP;

    private static final String reportName = "ViewPlayReportMain";
    ...
    public int run(String[] args) throws Exception {
        ...
        //输入路径        
        List<Path> inputs = PlayViewCollectSource.get().getInputs(startDate, userName, getConf());
    ...
    public void process(Iterable<Path> inputs, Date startDate) throws Exception {
        ...
        job.setJobName(ViewPlayReportMain.class.getSimpleName() + ":" + GnomeDateFormat.FORMAT_TILL_DATE.format(startDate));
        job.setMapperClass(MRPlayView.M1.class);
        job.setCombinerClass(MRPlayView.C1.class);
        job.setReducerClass(MRPlayView.R1.class);
...
//            }
        ...
        Path outputPath = new Path(new Path(ReportLocationConstants.REPORT_MEDUIM),
                "playReport/" + GnomeDateFormat.FORMAT_TILL_DATE_C.format(startDate));
    ...
    public static void main(String[] args) throws Exception {
        int res = OozieToolRunner.run(null, new ViewPlayReportMain(), args);
        System.exit(res);
