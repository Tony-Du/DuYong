...
public class DenoIncomeCollectSourceTest implements Source {
  
  private static DenoIncomeCollectSourceTest cs = new DenoIncomeCollectSourceTest();
  
  private DenoIncomeCollectSourceTest() {
    
  }
  
  public final static String base = "public/cdmp";
  
  public static DenoIncomeCollectSourceTest get() {
    return cs;
  }
  ...
  public List<Path> getInputs(Path base, Date startDate, Configuration conf, String taskName) throws IOException {
    ...
    if (base == null) base = new Path(DenoIncomeCollectSourceTest.base);
    ...
    for (FileStatus fv : fvs) {
    	...
    	Path timep = new Path(fv.getPath(), "20160502");
    	lst.add(timep);
    	Path timep2 = new Path(fv.getPath(), "20160501");
    	lst.add(timep2);
    	}
