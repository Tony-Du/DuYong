...
public class MeduimTableCollectSource {

    private static MeduimTableCollectSource cs = new MeduimTableCollectSource();

    private MeduimTableCollectSource() {

    }

    private final static String BASE_RELATIVE_DIRECTORY = ReportLocationConstants.MIGU_ORIGIN;

    public static MeduimTableCollectSource get() {
        return cs;
    }
    ...
    public List<Path> getInputs(Path base, Date startDate, Configuration conf) throws IOException {
        ...
        if (base == null)
            base = new Path(MeduimTableCollectSource.BASE_RELATIVE_DIRECTORY);
