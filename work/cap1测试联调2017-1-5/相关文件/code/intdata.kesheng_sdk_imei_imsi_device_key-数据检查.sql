...
CREATE VIEW `int.kesheng_sdk_session_start_cpa_v` AS select `a1`.`imei`, `a1`.`user_id`, `a1`.`idfa`, `a1`.`imsi`, `a1`.`app_ver_code`, `a1`.`phone_number`
      ,`a1`.`app_pkg_name`
  ...
  from (select  if(nvl(trim(`a`.`imei`),'') = '', '-998', trim(`a`.`imei`)) `imei`, 
                ...
                nvl(trim(regexp_extract(`a`.`first_launch_channel_id`,'([^-]+$)',1)),'-998') `app_channel_id`,
                `a`.`os` as `app_os_type`,
                `a`.`start_unix_time`,
                `a`.`install_id`,
                `a`.`src_file_day`,
