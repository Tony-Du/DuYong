...
——总共3个过程，数据源有td_aaa_bill_d, td_aaa_order_d, td_aaa_order_log_d，目标表有tm_month_fee_exception, tm_month_fee_info_d，一个过程更新month_fee_exception，另两个分别根据order_d, order_log_d更新month_fee_info_d								
...
begin								
    ...
    delete from cdmp_mk.tm_month_fee_exception_bak t where t.statis_month = iv_month;								
    ...
    vv_task_pos := '包月费用异常表';								
    insert into cdmp_mk.tm_month_fee_exception	--异常基础表
    (								
            serv_number,								
            product_id,								
            is_add,								
    ...
    select /*+ parallel(a,16) */ serv_number,								
            product_id,								
            is_add,			--标识是否本月新增订购关系					
            iv_month								
     ...
     where a.statis_day between iv_month||'01'								
       ...
       and not exists (select /*+ parallel(t,16) */1	--如果根据订购关系和订购日志判断要扣费,但是在boss回传话单中没有,则标记为异常,插入异常订购关系表
       ...
       and not exists (select 1 from cdmp_mk.tm_month_fee_exception b	--如果在上月的异常表中有，则本月继续标记为异常
                        where b.serv_number = a.serv_number								
                          and b.product_id = a.product_id );								
    commit;								
    delete /*+ parallel(a,16) */from cdmp_mk.tm_month_fee_exception a	--如果上个月boss扣到了费,则从异常表中把订购关系删除
    ...
    --得到数据量								
    vi_result := sql%rowcount;	-- sql%rowcount 中的 sql 是oracle的内部游标,rowcount的意思是之前的dml sql语句影响的数据有多少行						
    --提交事务								
    commit;								
...
(1)当数据日期=统计日期时，删除 cdmp_mk.tm_month_fee_exception_bak (异常包月费用备份表)
(2)备份异常包月费用表：cdmp_mk.tm_month_fee_exception |-> cdmp_mk.tm_month_fee_exception_bak
(3)包月费用异常表 cdmp_mk.tm_month_fee_info_d	   				|-> cdmp_mk.tm_month_fee_exception
				  cdmp_dw.td_aaa_bill_d       				    |
				  cdmp_mk.tm_month_fee_exception (统计日期之前) |
(4)删除 cdmp_mk.tm_month_fee_exception 中的 在 cdmp_dw.td_aaa_bill_d 非异常的的数据 
...
begin									
    ...
    if vi_result = 1 then									
	      if substr(iv_day,-2) = '01' then		--如果是某月的第一天，删除统计当日新增数据，并且删除统计当日非新增但data_source_flag = '1'的数据
            ...
            delete from cdmp_mk.tm_month_fee_info_d t where t.statis_day =  iv_day and is_add = '0' and data_source_flag = '1';									
		   	    ...
		   	    execute immediate 'alter table cdmp_mk.tm_month_fee_info_d truncate partition part_'||iv_day; --清空表分区旧数据				
		    end if;							
    else									
    	  execute immediate 'alter table cdmp_mk.tm_month_fee_info_d add partition part_'||iv_day||' values('''||iv_day||''')';	--建分区							
    end if;									
									
    vv_task_pos := '处理订购日志用户';									
    insert into cdmp_mk.tm_month_fee_info_d nologging	--当月新增收入
    (									
           statis_day,									
           prov_id,									
           region_id,									
    ...
    select /*+ parallel(t3,16) */statis_day,									
     ...
     from (select /*+ parallel(t,16) */iv_day statis_day,									
                   ...
                   t.new_product_id									
              from cdmp_dw.td_aaa_order_log_d t,cdmp_dw.td_pms_product_d t1		--先笛卡尔积，然后where再过滤			  
             where t.statis_day = iv_day 
               ...
               and not exists (select 1 from cdmp_mk.tm_month_fee_exception t3	--如果不在异常表，且不在当月的模拟扣费表里，则插入模拟扣费表
    ...
    select /*+ parallel(t,16) */iv_day statis_day,									
     ...
     where t.statis_day = to_char(to_date(iv_day,'yyyymmdd') - 1,'yyyymmdd')									
       ...
       and t.business_id not in (select aa.business_id									
                                   from cdmp_ods.to_business_info aa									
                                  where aa.business_class = '9')									
       and t1.product_price > 0		--如果截止昨天还在订，当前模拟扣费话单、异常表里都没有，日志中有复机记录，则插入模拟扣费话单					
       and not exists (select 1 from cdmp_mk.tm_month_fee_info_d t2									
                        where t2.statis_day between substr(iv_day,1,6)||'01' and to_char(to_date(iv_day,'yyyymmdd')-1,'yyyymmdd')									
                          and t.serv_number = t2.serv_number									
                          and t.product_id = t2.product_id )									
       and (t.serv_number,t.product_id) in (select t3.serv_number,t3.product_id									
                                              from cdmp_dw.td_aaa_order_log_d t3	-- 日志中有复机记录
...
begin										
    ...
    if vi_result = 1 then										
        delete from cdmp_mk.tm_month_fee_info_d t where t.statis_day =  v_net_month and is_add = '0' and data_source_flag = '2';										
    	  ...
    	  execute immediate 'alter table cdmp_mk.tm_month_fee_info_d add partition part_'||v_net_month||' values('''||v_net_month||''')';									
    end if;										
										
		insert into cdmp_mk.tm_month_fee_info_d nologging	--当月存量收入
    (										
           statis_day,										
           prov_id,										
           region_id,										
    ...
    select v_net_month statis_day,										
           ...
           '2' data_source_flag										
      from cdmp_dw.td_aaa_order_fact_d t,cdmp_dw.td_pms_product_d t1										
     where t.statis_day = to_char(last_day(to_date(iv_month,'yyyymm')),'yyyymmdd')	--如果上月末最后一天在订，且不在异常表里，则插入
