...
set mapreduce.job.name=rptdata.fact_ugc_order_user_num_monthly${SRC_FILE_MONTH};

set hive.groupby.skewindata=false;
set hive.optimize.skewjoin=true;
set hive.merge.mapredfiles = true;
insert overwrite table rptdata.fact_ugc_order_user_num_monthly partition (src_file_month='${SRC_FILE_MONTH}')
select 
	...
	SUM(case when (bb.third_add_month_order_user_flag = 'Y') then 1 else 0 end) as third_add_month_order_user_num,
	SUM(case when (bb.third_add_time_order_user_flag = 'Y') then 1 else 0 end) as third_add_time_order_user_num,
	SUM(case when (bb.third_prepay_user_flag = 'Y') then 1 else 0 end) as third_prepay_user_num,
	SUM(case when (bb.third_month_share_pay_user_flag = 'Y') then 1 else 0 end) as third_month_share_pay_user_num,
	SUM(case when (bb.third_pay_user_flag = 'Y') then 1 else 0 end) as third_pay_user_num,
	SUM(case when (bb.add_month_order_user_flag = 'Y') then 1 else 0 end) as add_month_order_user_num,
	SUM(case when (bb.add_time_order_user_flag = 'Y') then 1 else 0 end) as add_time_order_user_num,
	SUM(case when (bb.month_in_order_user_flag = 'Y') then 1 else 0 end) as month_in_order_user_num,
	SUM(case when (bb.boss_pay_user_flag = 'Y' or bb.third_pay_user_flag ='Y') then 1 else 0 end) as pay_user_num,
...
select 
	...
	from (
		select 
			...
			case when (a.period_unit = 'HOUR' and a.pay_chn_type in (50,58,59,302,304,306) and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as boss_time_add_order_user_flag,
			'N' as boss_month_in_order_user_flag,
			case when (a.authorize_type = 'BOSS_MONTH' and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as boss_month_add_order_user_flag,
			case when (a.authorize_type = 'BOSS_MONTH' and a.order_status in (12,14,15)) then 'Y' else 'N' end as boss_month_cancel_order_user_flag,
			--'N' as boss_month_add_and_cancel_user_flag,
			--'N' as boss_month_pay_user_flag,
			case when (a.period_unit = 'MONTH' and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_add_month_order_user_flag,
			case when (a.period_unit = 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status = 5 and  a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_add_time_order_user_flag,
			case when (a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_prepay_user_flag,
			'N' as third_month_share_pay_user_flag,
			case when (a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.order_status = 5 and a.pay_chn_type not in (50,58,59,302,304,306)) then 'Y' else 'N' end as third_pay_user_flag,
			case when (a.period_unit = 'MONTH' and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as add_month_order_user_flag,
			case when (a.period_unit = 'HOUR' and a.order_status = 5 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as add_time_order_user_flag,
			'N' as month_in_order_user_flag,
			--'N' as pay_user_flag,
			case when (a.period_unit = 'MONTH' and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.main = 0 and a.pay_chn_type in (50,58,59,302,304,306) and a.order_status = 5) then 'Y' else 'N' end as boss_pay_user_flag,
			case when (a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.main = 0 and a.pay_chn_type in (50,58,59,302,304,306) and a.order_status = 5) then 'Y' else 'N' end as boss_pay_active_user_flag,
			case when (a.authorize_type = 'BOSS_MONTH' and a.order_status = 5 and g.chn_attr_2_name = '省公司') then 'Y' else 'N' end as prov_develop_boss_add_order_user_flag,
		    case when (a.authorize_type = 'BOSS_MONTH' and a.order_status = 5 and g.chn_attr_2_name <> '省公司') then 'Y' else 'N' end as natural_develop_boss_add_order_user_flag,
		...
		where a.src_file_day like '${SRC_FILE_MONTH}%'
		...
		left outer join rptdata.dim_chn g
		on a.chn_id = g.chn_id --and g.dw_delete_flag = 'N'
		where a.src_file_day = '${SRC_FILE_END_MONTH_DAY}' --上个月最后一天快照，理想付费用户数使用
		...
		select 
			...
			a.user_id as user_id,
			'N' as boss_time_add_order_user_flag,
			case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null) then 'Y' else 'N' end as boss_month_in_order_user_flag,
			'N' as boss_month_add_order_user_flag,
			'N' as boss_month_cancel_order_user_flag,
			--'N' as boss_month_add_and_cancel_user_flag,
			--'N' as boss_month_pay_user_flag,
			...
			'N' as add_month_order_user_flag,
			'N' as add_time_order_user_flag,
			case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null) then 'Y' else 'N' end as month_in_order_user_flag,
			--'N' as pay_user_flag,
			case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null) then 'Y' else 'N' end as boss_pay_user_flag,
			case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null) then 'Y' else 'N' end as boss_pay_active_user_flag,
			'N' as prov_develop_boss_add_order_user_flag,
		    'N' as natural_develop_boss_add_order_user_flag,
		    case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null and g.chn_attr_2_name = '省公司') then 'Y' else 'N' end as prov_develop_boss_month_in_order_user_flag,
		    case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null and g.chn_attr_2_name <> '省公司') then 'Y' else 'N' end as natural_develop_boss_month_in_order_user_flag,
		...
		left outer join rptdata.dim_chn g
		on a.chn_id = g.chn_id --and g.dw_delete_flag = 'N'
		where a.src_file_day = '${SRC_FILE_DAY}' --本月最后一天快照，计算在订用户数
		...
		select 
			...
			'N' as third_add_time_order_user_flag,
			'N' as third_prepay_user_flag,
			'Y' as third_month_share_pay_user_flag,
			'Y' as third_pay_user_flag,
			'N' as add_month_order_user_flag,
			'N' as add_time_order_user_flag,
			'Y' as month_in_order_user_flag,
			--'N' as pay_user_flag,
			'N' as boss_pay_user_flag,
			'N' as boss_pay_active_user_flag,
			'N' as prov_develop_boss_add_order_user_flag,
		    ...
		    'N' as idea_pay_user_flag,
	        'N' as active_visit_user_flag
	    from rptdata.fact_ugc_tpp_contract_detail_monthly a
		left outer join rptdata.dim_server h
		on a.sub_busi_id = h.sub_busi_id --and h.dw_delete_flag = 'N'
		left outer join rptdata.dim_dept_term_prod b --<部门-终端产品维表> b
		on a.term_prod_id = b.term_prod_id  --and b.dw_delete_flag = 'N'
		...
		select 
			...
			'1' as user_type_id,
			nvl(f.prov_id, '-998') as province_id,
			a.phone_city_id as city_id,
           	nvl(f.prov_id, '-998') as phone_province_id,
			a.phone_city_id as phone_city_id,
			a.company_id as company_id,
			nvl(g.chn_id, '-998') as chn_id,
			nvl(g.chn_attr_1_id, '-998') as chn_attr_1_id,
			nvl(g.chn_attr_2_id, '-998') as chn_attr_2_id,
		    ...
		    'N' as natural_develop_boss_month_in_order_user_flag,
		    'N' as idea_pay_user_flag,
	        'Y' as active_visit_user_flag
        from rptdata.fact_active_user_detail_daily a
		left outer join rptdata.dim_server h
		on a.sub_busi_id = h.sub_busi_id --and h.dw_delete_flag = 'N'
		left outer join rptdata.dim_dept_term_prod b --<部门-终端产品维表> b
		on a.term_prod_id = b.term_prod_id  --and b.dw_delete_flag = 'N'
		...
		on a.term_prod_id = e.term_prod_id --and e.dw_delete_flag = 'N'
		left outer join rptdata.dim_region f
		on a.phone_city_id = f.region_id --and f.dw_delete_flag = 'N'
		left outer join rptdata.dim_chn g
		on a.chn_id = g.chn_id --and g.dw_delete_flag = 'N'
		where a.src_file_day like '${SRC_FILE_MONTH}%'
	) tmp
	group by dept_id,term_prod_id,term_video_type_id,term_video_soft_id,term_prod_type_id,term_version_id,term_os_type_id,busi_id,sub_busi_id,
    	net_type_id,cp_id,product_id,content_id,content_type,broadcast_type_id,user_type_id,province_id,city_id,phone_province_id, phone_city_id, company_id,chn_id,
    	chn_attr_1_id,chn_attr_2_id,chn_attr_3_id,chn_attr_4_id, chn_type,con_class_1_name, copyright_type, busi_type_id, program_id, user_id
	grouping sets (
			-- 核心
			(dept_id, user_id), 
			...
			(dept_id, busi_id, company_id, chn_id, chn_attr_1_id, chn_attr_2_id, user_id),
			...
			(dept_id, busi_id, phone_province_id, company_id, user_type_id, chn_id, chn_attr_1_id, chn_attr_2_id, user_id),
			(dept_id, term_video_type_id, company_id, user_id),
			(dept_id, term_video_type_id, company_id, user_type_id, user_id),
			(dept_id, busi_id, phone_province_id, company_id, chn_id, chn_attr_1_id, chn_attr_2_id, user_id),
			-- 地域
...
group by dept_id,term_prod_id,term_video_type_id,term_video_soft_id,term_prod_type_id,term_version_id,term_os_type_id,busi_id,sub_busi_id,
    net_type_id,cp_id,product_id,content_id,content_type,broadcast_type_id,user_type_id,province_id,city_id,phone_province_id, phone_city_id, company_id,chn_id,
    chn_attr_1_id,chn_attr_2_id,chn_attr_3_id,chn_attr_4_id, chn_type,con_class_1_name, copyright_type, busi_type_id, program_id, dim_group_id
;
