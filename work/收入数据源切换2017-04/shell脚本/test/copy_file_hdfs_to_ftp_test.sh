...
if [[ $# -ne 6 ]]; then
  echo "Usage: $FUNCNAME from_hdfs_path to_local_path to_file_name upload_ftp_path ftp_user_name ftp_password"
  exit 1
fi

set -e

hdfs_path="$1"	#/user/hive/warehouse/app.db/cpa_settle_bill_file_daily/src_file_day='20170409' 
local_path="$2"	#cpa/bigdata/settle 
file_name="$3"	#cpa_record_20170409
ftp_path="$4"
create_time=$(date '+%Y%m%d%H%M%S')

FTP_SERVER="10.200.65.71"
FTP_PORT="21"
FTP_USER="$5"  #"thadoop"
FTP_PWD="$6"   #"thadoop123!@#"
...
hdfs dfs -getmerge ${hdfs_path}/* ${local_path}/${file_name}.temp	#cpa_record_20170409.temp

file_bytes=`wc -c ${local_path}/${file_name}.temp | awk -F " " {'print $1'}`
echo "The file '${file_name}.temp' size:${file_bytes}"
