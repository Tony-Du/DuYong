...
drop table if exists app.cpa_sec_event_occur_monthly;

create table app.cpa_sec_event_occur_monthly(
product_key     string,
product_name    string,
app_ver_code    string,
...
-- ===================================================================
set mapreduce.job.name=app.cpa_sec_event_occur_monthly_${MONTH_START_DAY}_${MONTH_END_DAY};

insert overwrite table app.cpa_sec_event_occur_monthly partition (src_file_month = '${MONTH_START_DAY}')
select t1.product_key
  ...
  from( 
		...
		where a.src_file_month >= '${MONTH_START_DAY}'  
		  and a.src_file_month <= '${MONTH_END_DAY}'	-- tostring(adddays(addmonths(todate('${MONTH_START_DAY}', 'yyyyMMdd'), 1),-1) ,'yyyyMMdd')
