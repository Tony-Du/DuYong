...
drop table if exists app.cpa_sec_event_params_monthly;

create table app.cpa_sec_event_params_monthly(
product_key    string,
product_name   string,
app_ver_code   string,
...
-- =======================================================================
with stg_cpa_sec_event_params_monthly as 	-- 一个月内排名前1000位的params_val
(
...
insert overwrite table app.cpa_sec_event_params_monthly partition (src_file_month = '${MONTH_START_DAY}')
select t1.product_key
  ...
  from (
		select if(substr(a.grain_ind,1,1)='0', '-1', a.app_channel_id) as app_channel_id 
		  ...
		  from rptdata.fact_kesheng_sec_event_params_hourly a 
		  left join stg_cpa_sec_event_params_monthly b 
		       on a.event_name = b.event_name and a.param_name = b.param_name and a.param_val = b.param_val 
		 where a.src_file_day >= '${MONTH_START_DAY}'
		   and a.src_file_day <= '${MONTH_END_DAY}'	-- tostring(adddays(addmonths(todate('${MONTH_START_DAY}', 'yyyyMMdd'), 1),-1) ,'yyyyMMdd')
