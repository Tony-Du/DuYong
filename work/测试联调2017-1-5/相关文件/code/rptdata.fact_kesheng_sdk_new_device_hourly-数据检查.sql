...
CREATE VIEW ods.kesheng_sdk_sessionstart_v AS SELECT concat(s.input__file__name, ':', s.block__offset__inside__file) as rowkey,
       `d`.`os`,
       `d`.`imei`,
       `d`.`imsi`,
       `d`.`apppackagename`,
...
t1.src_file_day,
t1.src_file_hour
FROM intdata.kesheng_sdk_session_start t1 LEFT JOIN mscdata.dim_kesheng_sdk_app_pkg t2 ON (t1.os = t2.app_os_type AND t1.app_pkg_name = t2.app_pkg_name)  -- 关联关系：操作系统&app包名
WHERE t1.src_file_day = '${SRC_FILE_DAY}'
and t1.src_file_hour = '${SRC_FILE_HOUR}'
)
INSERT OVERWRITE TABLE stg.fact_kesheng_sdk_new_device_hourly_01
