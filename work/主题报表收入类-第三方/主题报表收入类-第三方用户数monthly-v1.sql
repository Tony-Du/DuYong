...
select 
    ...
    from (
        select 
            ...
            case when (a.period_unit <> 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_add_month_order_user_flag,
            case when (a.period_unit = 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') and  a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_add_time_order_user_flag,
            case when (a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as third_prepay_user_flag,
            ...
            case when (a.period_unit = 'MONTH' and a.order_status in ('5','9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as add_month_order_user_flag,
            case when (a.period_unit = 'HOUR' and a.order_status in ('5','9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE')) then 'Y' else 'N' end as add_time_order_user_flag,
        ...
        select 
            ...
            'N' as add_month_order_user_flag,
            'N' as add_time_order_user_flag,
            case when (a.chrgprod_price > 0 and a.order_status <> '4' and a.sub_status is null) then 'Y' else 'N' end as month_in_order_user_flag,  --boss+第三方在定用户数（boss）
            --'N' as pay_user_flag,
        ...
        left outer join rptdata.dim_chn g
        on a.chn_id = g.chn_id --and g.dw_delete_flag = 'N'
        where substr(a.src_file_day,1,6) = '${SRC_FILE_MONTH}'
