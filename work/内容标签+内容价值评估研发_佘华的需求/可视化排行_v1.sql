...
--日期,省份,终端大类名称,内容ID,内容名称,原创发行,授权方式,NCP_ID,内容一级分类,版权类型,首发状态,使用次数

add jar hdfs:/user/hadoop/ods/udfjar/spark-streaming.jar;

set hive.auto.convert.join=false; --是否根据输入小表的大小，自动将 Reduce端的 Common Join 转化为 Map Join，从而加快大表关联小表的 Join 速度




create or replace view rpt.fact_visual_kesheng_sort_content_1_v as 
select a.src_file_day                                                --日期
  ...
  left join rptdata.dim_content_class d4        --内容分类维表
...
--付费产品维表： rptdata.dim_charge_product
         
--计费产品ID，product_id 关联 case when product_price > 0.1 then else end as fee_type,   


         
--日期,省份,终端大类名称,内容ID,内容名称,NCP-ID,内容一级分类,,版权类型,首发状态,使用次数
...
--日期,省份,终端大类名称,内容ID,内容名称,NCP-ID,内容一级分类,内容二级分类,内容标签值,版权类型,首发状态,使用次数

add jar hdfs:/user/hadoop/ods/udfjar/spark-streaming.jar;

set hive.auto.convert.join=false;

create or replace view rpt.fact_visual_kesheng_sort_content_3_v as
select a.src_file_day                                                --日期
      ,nvl(p.prov_name, '-998') as prov_name                         --省份
      ,nvl(d1.term_video_type_name, '-998') as term_video_type_name  --终端大类名称
      ,a.content_id                                                  --内容ID
      ...
      ,nvl(d4.con_class_1_name, '-998') as con_class_1_name          --一级分类名称
      ,nvl(d4.con_class_2_name, '-998')  as con_class_2_name         --二级分类名称
      ,nvl(d4.con_tag_1_name, '-998') as con_tag_1_name              --一级标签   
      ,case when d5.chrgprod_price is null then '-998' 
            when d5.chrgprod_price > 0.1 then '计费产品' else '免费产品' end as fee_type      
      ,count(distinct a.session) as use_cnt                          --使用次数
  from rptdata.fact_user_play_detail a
