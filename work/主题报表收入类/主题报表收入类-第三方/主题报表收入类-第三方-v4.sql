...
select  bb.dept_id as dept_id
       ...
       ,sum(case when (bb.third_pay_user_flag = 'Y') then 1 else 0 end) as third_pay_user_num                                               --天。第三方付费用户（包月分摊+按次）
       ,sum(case when (bb.mon_third_pay_user_flag = 'Y') then 1 else 0 end) as mon_third_pay_user_num                                       --月。第三方付费用户（包月分摊+按次）
  ...
  from (
        select tt.dept_id
              ...
              ,max(tt.third_pay_user_flag) as third_pay_user_flag                                               --天。第三方付费用户（包月分摊+按次）  
              ,max(tt.mon_third_pay_user_flag) as mon_third_pay_user_flag                                       --月。第三方付费用户（包月分摊+按次） 指标28
              ,max(tt.boss_third_add_period_order_user_flag) as boss_third_add_period_order_user_flag           --天。boss+第三方 新增包周期订购用户
              ,max(tt.mon_boss_third_add_period_order_user_flag) as mon_boss_third_add_period_order_user_flag   --月。boss+第三方 新增包周期订购用户
              ,max(tt.boss_third_add_time_order_user_flag) as boss_third_add_time_order_user_flag               --天。boss+第三方 新增按次订购用户
              ,max(tt.mon_boss_third_add_time_order_user_flag) as mon_boss_third_add_time_order_user_flag       --月。boss+第三方 新增按次订购用户
          ...
          from ( 
                select case when e.term_prod_name in ('和视频OPENAPI','和视频SDK') then nvl(d.dept_id,'-998') else nvl(b.dept_id,nvl(c.dept_id,nvl(d.dept_id,'-998'))) end as dept_id
                       ...
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and a.period_unit <> 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as third_add_period_order_user_flag       --天。第三方新增包周期订购用户
                       ,case when a.period_unit <> 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as mon_third_add_period_order_user_flag   --月。第三方新增包周期订购用户             
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and a.period_unit = 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as third_add_time_order_user_flag     --天。第三方新增按次订购用户
                       ,case when a.period_unit = 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as mon_third_add_time_order_user_flag --月。第三方新增按次订购用户          
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and  a.period_unit = 'hour' 
                              and a.main = 0 and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5','9') 
                       ...
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and a.period_unit <> 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as boss_third_add_period_order_user_flag      --天。boss+第三方 新增包周期订购用户            
                       ,case when a.period_unit <> 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as mon_boss_third_add_period_order_user_flag       --月。boss+第三方 新增包周期订购用户            
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and a.period_unit = 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as boss_third_add_time_order_user_flag     --天。boss+第三方 新增按次订购用户                 
                       ,case when a.period_unit = 'hour' 
					          and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.order_status in ('5','9') 
							 then 'Y' else 'N' end as mon_boss_third_add_time_order_user_flag --月。boss+第三方 新增按次订购用户
                ...
                select case when e.term_prod_name in ('和视频OPENAPI','和视频SDK') then nvl(d.dept_id,'-998') else nvl(b.dept_id,nvl(c.dept_id,nvl(d.dept_id,'-998'))) end as dept_id
                       ...
                       ,case when t.src_file_day = '${SRC_FILE_DAY}' and t.expire_time >= '${SRC_FILE_DAY}' and t.start_time <= '${SRC_FILE_DAY}'
                             then 'Y' else 'N' end as boss_third_month_in_order_user_flag  --天。boss+第三方包月在订用户(第三方)    
                ...
                select case when e.term_prod_name in ('和视频OPENAPI','和视频SDK') then nvl(d.dept_id,'-998') else nvl(b.dept_id,nvl(c.dept_id,nvl(d.dept_id,'-998'))) end as dept_id
                       ...
                       ,'N' as boss_third_add_period_order_user_flag      --天。boss+第三方 新增包周期订购用户
                       ,'N' as mon_boss_third_add_period_order_user_flag  --月。boss+第三方 新增包周期订购用户      
                       ,'N' as boss_third_add_time_order_user_flag        --天。boss+第三方 新增按次订购用户
                       ,'N' as mon_boss_third_add_time_order_user_flag    --月。boss+第三方 新增按次订购用户  
                       ,case when a.src_file_day = '${SRC_FILE_DAY}' and a.chrgprod_price > 0 and a.sub_status is null and a.order_status <> '4' 
							 then 'Y' else 'N' end as month_in_order_user_flag   --天。boss+第三方 包月在订用户(boss) 
                       ,0 as sale_revenue 		--天。销售收入 
					   ,0 as mon_sale_revenue	--月。销售收入     
