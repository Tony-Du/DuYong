...
select
     ...
     --0 as total_amount,
     -- 新增部分
     --0 as tpp_real_amount,
     0 as tpp_add_period_amount,
     0 as tpp_subside_amount,
     0 as tpp_add_time_amount,
     0 as sales_amount
...
select
     ...
     0 as boss_month_add_info_fee_amount,
     0 as boss_month_retention_info_amount,
     case when (a.period_unit = 'HOUR' and a.pay_chn_type in (50,58,59,302,304,306) and a.order_status in ('5', '9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.main = 0) then a.amount else 0 end as boss_time_amount,
     0 as boss_month_info_amount,
     case when (a.period_unit = 'HOUR' and a.pay_chn_type in (50,58,59,302,304,306) and a.order_status in ('5', '9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.main = 0) then a.amount else 0 end as boss_time_info_amount,
     case when (a.pay_chn_type not in (50,58,59,302,304,306) and a.order_status in ('5', '9') and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') and a.main = 0) then a.amount else 0 end as third_prepay_Amount,
     0 as real_amount,
     --0 as total_amount,
...
select
     ...
     0 as boss_time_info_amount,
     0 as third_prepay_Amount,    
     case when (a.period_unit = 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) 
                                       and a.order_status in ('5', '9') 
                                       and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') 
                                       and a.main = 0) then a.amount else 0 end as real_amount,	-- 第三方实际收入（包月分摊+按次）
     --0 as total_amount,
     ...
     case when (a.period_unit <> 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) 
                                        and a.order_status in ('5', '9') 
                                        and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') 
                                        and a.main = 0) then a.amount else 0 end as tpp_add_period_amount,
     0 as tpp_subside_amount,
     case when (a.period_unit = 'HOUR' and a.pay_chn_type not in (50,58,59,302,304,306) 
                                       and a.order_status in ('5', '9') 
                                       and a.goods_type not in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') 
                                       and a.main = 0) then a.amount else 0 end as tpp_add_time_amount,
     case when (a.order_status in ('5', '9') and a.goods_type in ('MIGU_MOVIE_CARD','MOVIE_CARD_DELAY','MIGU_LIVE') 
                                             and a.main = 0) then a.amount else 0 end as sales_amount
...
select
     ...
     0 as boss_time_info_amount,
     0 as third_prepay_Amount,
     period_amount as real_amount,	 -- 第三方实际收入(包月分摊+按次)
	 --0 as total_amount,
     ...
     0 as tpp_add_time_amount,
     0 as sales_amount
 from rptdata.fact_ugc_tpp_order_relation_detail a
where src_file_day = '${SRC_FILE_DAY}'
  and unix_timestamp(src_file_day, 'yyyyMMdd') >= valid_start_time
  and unix_timestamp(src_file_day, 'yyyyMMdd') <= expire_time
...
-- 新增部分
--sum(tpp_real_amount) as tpp_real_amount,
sum(tpp_add_period_amount) as tpp_add_period_amount,
sum(tpp_subside_amount) as tpp_subside_amount,
sum(tpp_add_time_amount) as tpp_add_time_amount,
...
--sum(case when mobile_city_id <> '-998' then tpp_real_amount else 0 end) as tpp_real_amount,
sum(case when mobile_city_id <> '-998' then tpp_add_period_amount else 0 end) as tpp_add_period_amount,
sum(case when mobile_city_id <> '-998' then tpp_subside_amount else 0 end) as tpp_subside_amount,
sum(case when mobile_city_id <> '-998' then tpp_add_time_amount  else 0 end) as tpp_add_time_amount,
sum(case when mobile_city_id <> '-998' then sales_amount else 0 end) as sales_amount,
