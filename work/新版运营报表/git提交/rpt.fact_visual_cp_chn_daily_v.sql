create or replace view rpt.fact_visual_cp_chn_daily_v as 
select 
     case when B.dim_group_id & cast(power(2, 7) as bigint) > 0 then nvl(C.business_name, '未知') 
          else '剔重汇总' end as business_name,
     --power(2,7):计算2的7次幂，返回一个double类型的数，cast(power(2, 7) as bigint)将返回值转化为bigint类型
      ...
      select src_file_day,
        ...
        from (
              select src_file_day,
               ...
               where dept_id='D04'
                 and lpad(bin(dim_group_id), 31, '0') = '0000000111101000000000010000001' 
                 --从左边开始，以0填充，直至31位  lpad(bin(15990913), 31, '0')=0000000111101000000000010000001
              ...
              select src_file_day,
                ...
                from rptdata.cdmp_rpt_cnt_daily
               where dept_id='D04'
                 and lpad(bin(dim_group_id), 31, '0') = '0000000111101000000000010000001'
             ) all
        group by src_file_day, busi_id, sub_busi_id, term_video_type_id, province_id,
                 company_id, user_type_id, chn_id, chn_attr_1_id, chn_attr_2_id,
                 dim_group_id
)B
