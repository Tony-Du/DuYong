...
case when B.dim_group_id & cast(power(2, 7) as bigint) > 0 then nvl(C.business_name, '未知')
...
cast(from_unixtime(unix_timestamp(B.src_file_day,'yyyyMMdd')) as timestamp)  AS statis_date,
B.dim_group_id,
B.src_file_day,
B.busi_id,
B.sub_busi_id,
...
from (
		...
		from rptdata.cdmp_rpt_user_num_daily 
		where src_file_day>='20161201' 
		and dept_id='D04'
		and lpad(bin(dim_group_id), 31, '0')  in	--从左边开始，以0填充，直至31位
		('0000000000100001000000010000001',      	-- =lpad(bin(15728769), 31, '0')
		'0000000000100000000000010000001',
		'0000000000100000000000110000001',
		'0000000111100000000000010000001',
		'0000000000101000000000000000001',
		'0000000000101001000000010000001',
		'0000000000101000000000010000001',			--0000000111101000000000010000001
		'0000000111101001000000010000001',
		'0000000000100000000000000000101',
		'0000000000100001000000000000101',
		'0000000111101000000000010000001')
		...
		from 
		rptdata.cdmp_rpt_cnt_daily
		where src_file_day>='20161201' 
		and dept_id='D04'
		and lpad(bin(dim_group_id), 31, '0')  in
		('0000000000100001000000010000001',      
		'0000000000100000000000010000001',
		'0000000000100000000000110000001',
		'0000000111100000000000010000001',
		'0000000000101000000000000000001',
		'0000000000101001000000010000001',
		'0000000000101000000000010000001',
		'0000000111101001000000010000001',
		'0000000000100000000000000000101',
		'0000000000100001000000000000101',
		'0000000111101000000000010000001')
...
left join 
(select distinct business_id,business_name from rptdata.dim_server) C
on B.busi_id=C.business_id
left join 
(select distinct sub_busi_id,sub_busi_name from rptdata.dim_server) D
on B.sub_busi_id=D.sub_busi_id
left join 
(select distinct business_id,busi_type from  rptdata.dim_comp_busi) F
on B.busi_id=F.business_id
left join 
(select distinct prov_id,prov_name from rptdata.dim_region) G
on B.province_id=G.prov_id
left join 
rpt.dim_term_prod_video_type_v H
on B.term_video_type_id=H.term_video_type_id
left join 
(select  distinct comp_id,comp_name  from rptdata.dim_comp_busi) I
on B.company_id=I.comp_id
left join 
(select  distinct chn_id,chn_name  from rptdata.dim_chn) J
on B.chn_id=J.chn_id

left join 
(select distinct chn_attr_1_id,chn_attr_1_name from rptdata.dim_chn) K
on B.chn_attr_1_id=K.chn_attr_1_id
left join
(select distinct chn_attr_2_id,chn_attr_2_name from rptdata.dim_chn) L
on B.chn_attr_2_id=L.chn_attr_2_id

left join 
(select  distinct user_type_id,user_type_name  from rptdata.dim_user_type) E
